...
`
client/build/main -op=<операция-над-хранилищем> -key=<ключ> -val=<значение> 
...
```
client/build/main -op=add -key=somekey -val=somevalue -ttl=10m
```
...
```
client/build/main -op=set -key=somekey -val=foobar 
```
...
```
client/build/main -op=get -key=somekey
...
```
client/build/main -op=del -key=somekey
